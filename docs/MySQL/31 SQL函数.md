...
![image-20230510234822279](/images/MySQL/image-20230510234822279.png)
...
![image-20230510235031431](/images/MySQL/image-20230510235031431.png)
...
![image-20230510235412016](/images/MySQL/image-20230510235412016.png)
...
mysql> SELECT EXTRACT(YEAR FROM '2023-02-16');
+---------------------------------+
| EXTRACT(YEAR FROM '2023-02-16') |
+---------------------------------+
|                            2023 |
+---------------------------------+
1 row in set (0.00 sec)

mysql> SELECT DATE('2023-02-06 12:00:05');
+-----------------------------+
| DATE('2023-02-06 12:00:05') |
+-----------------------------+
| 2023-02-06                  |
+-----------------------------+
```
...
![image-20230511000328486](/images/MySQL/image-20230511000328486.png)
...
> 显示英雄最大生命值的最大值，就需要用到 MAX 函数。在数据中，“最大生命值”对应的列数为`hp_max`，在代码中的格式为`MAX(hp_max)`
...
> 想要知道最大生命值最大的是哪个英雄，以及对应的数值，就需要分成两个步骤来处理：首先找到英雄的最大生命值的最大值，即`SELECT MAX(hp_max) FROM heros`，然后再筛选最大生命值等于这个最大值的英雄
...
> 显示英雄的名字，以及他们的名字字数，需要用到`CHAR_LENGTH`函数
...
> 提取英雄上线日期（对应字段 birthdate）的年份，只显示有上线日期的英雄即可（有些英雄没有上线日期的数据，不需要显示），这里我们需要使用 EXTRACT 函数，提取某一个时间元素。所以我们需要筛选上线日期不为空的英雄，即`WHERE birthdate is not null`，然后再显示他们的名字和上线日期的年份
...
> 找出在 2016 年 10 月 1 日之后上线的所有英雄。这里我们可以采用 DATE 函数来判断 birthdate 的日期是否大于 2016-10-01，即`WHERE DATE(birthdate)>'2016-10-01'`，然后再显示符合要求的全部字段信息
...
> 知道在 2016 年 10 月 1 日之后上线英雄的平均最大生命值、平均最大法力和最高物攻最大值。同样我们需要先筛选日期条件，即`WHERE DATE(birthdate)>'2016-10-01'`，然后再选择`AVG(hp_max), AVG(mp_max), MAX(attack_max)`字段进行显示。
...
[heros_data_with_structure.sql](https://github.com/cystanford/sql_heros_data/blob/master/heros_data_with_structure.sql)
...
![image-20230511114109630](/images/MySQL/image-20230511114109630.png)
...
尽管 SQL 函数使用起来会很方便，但我们使用的时候还是要谨慎，因为你使用的函数很可能在运行环境中无法工作

DBMS 之间的差异性很大,比如，大多数 DBMS 使用（||）或者（+）来做拼接符，而在 MySQL 中的字符串拼接函数为`Concat()`。大部分 DBMS 会有自己特定的函数，这就意味着采用 SQL 函数的代码**可移植性是很差**的，因此在使用函数的时候需要特别注意
